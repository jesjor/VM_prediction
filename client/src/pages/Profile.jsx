  ...
  return (
    <div>
      ...
      <div style={{ padding:'1.25rem 0 1rem', display:'flex', alignItems:'center', gap:14 }}>
        <div style={{ width:52, height:52, borderRadius:'50%', background:getColor(participant.name), display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Barlow Condensed',sans-serif", fontSize:20, fontWeight:700, flexShrink:0 }}>
          ...
          {!t ? (
            ...
            <>
              ...
              <div className="card">
                ...
                <Row label="🃏 Flest kortpoint (spiller)" value={t.most_card_pts_player} />
                ...
                <Row label="🛡️ Færrest mål lukket ind" value={t.least_goals_conceded} />
      ...
      {tab==='stats' && (
        <div className="card">
          {!pStats || pStats.total_matches===0 ? (
            <div className="empty">Ingen afsluttede kampgæt endnu.</div>
          ) : (
            <>
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:'1rem'}}>
                {[
                  ['Rigtige resultater', `${pStats.result_hits}/${pStats.total_matches}`, `${pStats.result_pct}%`, 'var(--green)'],
                  ['Eksakte scores', pStats.exact_hits, 'gange', 'var(--gold)'],
                  ['Rigtige scorere', `${pStats.scorer_hits}/${pStats.total_matches}`, `${pStats.scorer_pct}%`, 'var(--blue)'],
                ].map(([label, big, small, color]) => (
                  <div key={label} style={{textAlign:'center',background:'var(--bg3)',borderRadius:8,padding:'10px 6px'}}>
                    <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:24,fontWeight:800,color}}>{big}</div>
                    <div style={{fontSize:11,color:'var(--text3)',marginTop:2}}>{small}</div>
                    <div style={{fontSize:11,color:'var(--text3)',marginTop:1}}>{label}</div>
                  </div>
                ))}
              </div>
              {pStats.best_match && (
                <div style={{background:'rgba(245,197,24,.08)',border:'1px solid rgba(245,197,24,.25)',borderRadius:8,padding:'10px 12px'}}>
                  <div style={{fontSize:12,color:'var(--gold)',fontWeight:600,marginBottom:4}}>🏆 Bedste kamp</div>
                  <div style={{fontSize:14,fontWeight:600}}>{pStats.best_match.home} {pStats.best_match.score} {pStats.best_match.away}</div>
                  <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:24,fontWeight:800,color:'var(--gold)'}}>{pStats.best_match.pts} pt</div>
                </div>
              )}
            </>
          )}
                  ...
                  <div key={m.id} className={`match-card${m.status==='finished'?' finished':''}`} style={{marginBottom:8}}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                      ...
                      <div style={{textAlign:'right'}}>
                        ...
                        <div style={{fontSize:13,display:'flex',gap:4,justifyContent:'flex-end',marginTop:2}}>
                          <span className={`badge ${pred.prediction==='1'?'badge-blue':pred.prediction==='X'?'badge-gray':'badge-green'}`}>{pred.prediction}</span>
                          {pred.exact_home!==null&&pred.exact_home!==undefined && <span className="badge badge-gold" style={{fontSize:10}}>{pred.exact_home}-{pred.exact_away??0}</span>}
                        </div>
                      </div>
                    </div>
