...
// POST add player to squad (admin)
...
// DELETE player from squad (admin)
...
router.put('/dream-team/:participantId', async (req, res) => {
  ...
  try {
    ...
    // Lock before tournament starts
    const lockTime = new Date('2026-06-11T18:45:00Z'); // 15 min before kickoff
...
// Admin: set dream team official result
router.put('/dream-team-result', requireAdmin, async (req, res) => {
  const { players, best_player } = req.body; // players: string[]
  try {
    await pool.query(`
      INSERT INTO tournament_results (result_key, player) VALUES ('dream_team_players', $1)
      ON CONFLICT (result_key) DO UPDATE SET player=$1, updated_at=NOW()
    `, [JSON.stringify(players)]);
    if (best_player) {
      await pool.query(`
        INSERT INTO tournament_results (result_key, player) VALUES ('dream_team_best_player', $1)
        ON CONFLICT (result_key) DO UPDATE SET player=$1, updated_at=NOW()
      `, [best_player]);
    }
    res.json({ ok: true });
